package airdock.util 
{
	...
	public class DockerFacilitator 
	{
		...
		public function setupWindows(...windows):void
		{
			function setupWindow(item:NativeWindow, ...rest):void {
				cl_targetDocker.setupWindow(item);
			}
			
			if (windows.length == 1 && windows[0] is Vector.<NativeWindow>) {
				(windows[0] as Vector.<NativeWindow>).forEach(setupWindow, cl_targetDocker);
			}
			else {
				windows.forEach(setupWindow, cl_targetDocker)
			}
		}
		
		public function unhookWindows(...windows):void
		{
			function unhookWindow(item:NativeWindow, ...rest):void {
				cl_targetDocker.unhookWindow(item);
			}
			
			if (windows.length == 1 && windows[0] is Vector.<NativeWindow>) {
				(windows[0] as Vector.<NativeWindow>).forEach(unhookWindow, cl_targetDocker);
			}
			else {
				windows.forEach(unhookWindow, cl_targetDocker);
			}
		}
